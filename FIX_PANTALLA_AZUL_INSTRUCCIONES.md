...
| 3 | **`streamlit_app.py` nuevo** (entry point canónico) | `streamlit_app.py` | Streamlit Cloud lo autodetecta y prioriza sobre `main.py` (que cargaba billing por error) |
...
- streamlit_app.py: nuevo entry point canonico (apunta a main_medicare, no a billing)
