...
def _fecha_registro_o_none(reg: Dict[str, Any]):
...
def _registro_en_rango_fechas(
    reg: Dict[str, Any],
    d_desde,
    d_hasta,
    incluir_sin_fecha: bool,
) -> bool:
    fd = _fecha_registro_o_none(reg)
    if fd is None:
        return incluir_sin_fecha
    return d_desde <= fd <= d_hasta


def _sort_registros_por_fecha(registros: List[Dict[str, Any]], recientes_primero: bool) -> List[Dict[str, Any]]:
    def clave(r):
...
def render_historial(paciente_sel: str) -> None:
    ...
    st.bar_chart(df_cnt.set_index("Sección")["Cantidad"], use_container_width=True)
    ...
    if mostrar_tl:
        ...
        if filas_tl:
            ...
            altura_tl = min(400, max(200, 40 + len(filas_tl) * 36))
    ...
    if q_global.strip():
        ...
        if not hits:
            ...
            altura_h = min(400, max(200, 40 + len(hits) * 36))
    ...
    if usar_fecha:
        ...
        registros_fecha = [
            r
            for r in registros
            if _registro_en_rango_fechas(r, d_desde, d_hasta, incluir_sin_fecha=incluir_sin_fecha)
        ]
    else:
