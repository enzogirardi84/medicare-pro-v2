...
def render_visitas(paciente_sel, mi_empresa, user, rol):
    ...
    agenda_db = st.session_state.setdefault("agenda_db", [])
    checkin_db = st.session_state.setdefault("checkin_db", [])
    ...
    if fichadas_hoy:
        ...
        for f in fichadas_hoy:
            ...
            if "LLEGADA" in f["tipo"].upper():
                ...
                horas, minutos = _duracion_humana(duracion)
        ...
        if llegada_time:
            ...
            horas, minutos = _duracion_humana(duracion_actual)
    ...
    if agenda_paciente:
        ...
        registros_accion = df_filtrado.sort_values("_fecha_dt").to_dict("records")
        opciones_accion = [
            f"{x['Fecha y Hora']} | {x['Profesional']} | {x['Estado']}"
            for x in registros_accion
        ]
        seleccion = c_a1.selectbox(
            "Accion rapida sobre una visita",
            range(len(opciones_accion) + 1),
            format_func=lambda i: "Sin cambios" if i == 0 else opciones_accion[i - 1],
            key=f"agenda_accion_sel_{paciente_sel}",
        )
        ...
        if st.button("Aplicar cambio de agenda", use_container_width=True, key=f"agenda_apply_{paciente_sel}"):
            if seleccion != 0:
                objetivo = registros_accion[seleccion - 1]
                if objetivo:
                    for item in agenda_db:
                        if (
