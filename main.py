...
APP_BUILD_TAG = f"Build {datetime.fromtimestamp(Path(__file__).stat().st_mtime).strftime('%Y-%m-%d %H:%M')}"
...
def render_current_view(tab_name, paciente_sel, mi_empresa, user, rol):
    ...
    try:
        ...
        st.caption(f"Modulo interno: {module_name}.{function_name}")
        st.caption(f"Build activo: {APP_BUILD_TAG}")
        return
...
def _sidebar_patient_card(paciente_sel, detalles):
    dni = escape(detalles.get("dni", "S/D"))
    cobertura = escape(detalles.get("obra_social", "S/D"))
    empresa = escape(detalles.get("empresa", "S/D"))
    estado = escape(detalles.get("estado", "Activo"))
    return (
        f'<div class="mc-patient-card">'
        f'<div class="mc-patient-card-head">'
        f'<div>'
        f'<div class="mc-patient-card-kicker">Paciente activo</div>'
        f'<div class="mc-patient-card-name">{escape(paciente_sel)}</div>'
        f'</div>'
        f'<span class="mc-patient-status-pill">{estado}</span>'
        f'</div>'
        f'<div class="mc-patient-chip-grid">'
        f'<div class="mc-patient-chip"><span>DNI</span><strong>{dni}</strong></div>'
        f'<div class="mc-patient-chip"><span>Cobertura</span><strong>{cobertura}</strong></div>'
        f'<div class="mc-patient-chip"><span>Empresa</span><strong>{empresa}</strong></div>'
        f"</div>"
        f"</div>"
    )
...
def _sidebar_brand_card(mi_empresa, user, rol, descripcion, logo_sidebar_b64):
    rol_txt = escape(rol or "Usuario")
    nombre = escape(user.get("nombre", ""))
    desc = escape(descripcion or "Panel operativo activo.")
    logo_html = (
    ...
    return (
        ...
        f'<div class="mc-brand-user">{nombre} <span>{rol_txt}</span></div>'
        f'<div class="mc-sidebar-pill-row">'
        f'<span class="mc-sidebar-status-pill">Operacion segura</span>'
        f'<span class="mc-sidebar-status-pill mc-sidebar-status-pill--soft">{rol_txt}</span>'
        f"</div>"
        f'<div class="mc-brand-copy">{desc}</div>'
        f'<div class="mc-sidebar-mini-grid">'
        f'<div class="mc-sidebar-mini-stat"><span>Estado</span><strong>En linea</strong></div>'
        f'<div class="mc-sidebar-mini-stat"><span>Panel</span><strong>Operativo</strong></div>'
        f"</div>"
        f"</div>"
    )
...
def render_module_nav(menu, vista_actual):
    if not menu:
        return None
    vista_label = escape(VIEW_NAV_LABELS.get(vista_actual, vista_actual))
    total_modulos = len(menu)
    st.markdown(
        f"""
        <section class="mc-module-shell" aria-label="Navegacion principal de modulos">
            <div class="mc-module-shell-head">
                ...
                <p class="mc-module-shell-copy">Cambia rapido entre areas sin perder contexto. La barra se adapta a touch, tablet y escritorio.</p>
            </div>
            <div class="mc-module-shell-grid">
                <div class="mc-module-feature">
                    <span class="mc-module-feature-kicker">Activo ahora</span>
                    <div class="mc-module-feature-title">{vista_label}</div>
                    <div class="mc-module-feature-copy">El contenido inferior se actualiza al instante al tocar otro modulo.</div>
                </div>
                <div class="mc-module-stat">
                    <span class="mc-module-stat-value">{total_modulos}</span>
                    <span class="mc-module-stat-label">modulos habilitados</span>
                </div>
                <div class="mc-module-stat">
                    <span class="mc-module-stat-value">Touch</span>
                    <span class="mc-module-stat-label">desliza en movil o usa clic directo en pantallas grandes</span>
                </div>
            </div>
        </section>
        """,
