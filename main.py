import base64
import json
import sys
from importlib import import_module
from pathlib import Path
import streamlit as st

# 1. CONFIGURACIÓN DE RUTAS
ROOT_DIR = Path(__file__).resolve().parent
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

# 2. IMPORTACIONES DINÁMICAS (CORE)
try:
    from core.auth import check_inactividad, render_login
    from core import utils as core_utils
    from core.database import cargar_datos
except ImportError as e:
    st.error(f"⚠️ Error crítico de dependencias: {e}")
    st.stop()

# 3. ASIGNACIÓN DE UTILIDADES
cargar_texto_asset = core_utils.cargar_texto_asset
es_control_total = core_utils.es_control_total
inicializar_db_state = core_utils.inicializar_db_state
obtener_modulos_permitidos = core_utils.obtener_modulos_permitidos
obtener_pacientes_visibles = core_utils.obtener_pacientes_visibles
obtener_alertas_clinicas = core_utils.obtener_alertas_clinicas
descripcion_acceso_rol = core_utils.descripcion_acceso_rol

# 4. CONFIGURACIÓN UI
st.set_page_config(
    page_title="MediCare Enterprise PRO V9.12", 
    layout="wide", 
    initial_sidebar_state="collapsed"
)

# Inyectar CSS global desde assets/style.css
css_content = cargar_texto_asset('style.css')
if css_content:
    st.markdown(f"<style>{css_content}</style>", unsafe_allow_html=True)

# Inicializar Base de Datos en sesión
if "_db_bootstrapped" not in st.session_state:
    inicializar_db_state(cargar_datos() if cargar_datos else None)
    st.session_state["_db_bootstrapped"] = True

# 5. DICCIONARIO DE MÓDULOS (VISTAS)
VIEW_CONFIG = {
    "Visitas y Agenda": ("views.visitas", "render_visitas"), 
    "Dashboard": ("views.dashboard", "render_dashboard"),
    "Admision": ("views.admision", "render_admision"), 
    "Clinica": ("views.clinica", "render_clinica"),
    "Pediatria": ("views.pediatria", "render_pediatria"), 
    "Evolucion": ("views.evolucion", "render_evolucion"),
    "Estudios": ("views.estudios", "render_estudios"), 
    "Materiales": ("views.materiales", "render_materiales"),
    "Recetas": ("views.recetas", "render_recetas"), 
    "Balance": ("views.balance", "render_balance"),
    "Inventario": ("views.inventario", "render_inventario"), 
    "Caja": ("views.caja", "render_caja"),
    "Emergencias y Ambulancia": ("views.emergencias", "render_emergencias"), 
    "Red de Profesionales": ("views.red_profesionales", "render_red_profesionales"),
    "Escalas Clinicas": ("views.escalas_clinicas", "render_escalas_clinicas"), 
    "Historial": ("views.historial", "render_historial"),
    "PDF": ("views.pdf_view", "render_pdf"), 
    "Telemedicina": ("views.telemedicina", "render_telemedicina"),
    "Cierre Diario": ("views.cierre_diario", "render_cierre_diario"), 
    "Mi Equipo": ("views.mi_equipo", "render_mi_equipo"),
    "Asistencia en Vivo": ("views.asistencia", "render_asistencia"), 
    "RRHH y Fichajes": ("views.rrhh", "render_rrhh"),
    "Auditoria": ("views.auditoria", "render_auditoria"), 
    "Auditoria Legal": ("views.auditoria_legal", "render_auditoria_legal"),
}

VIEW_NAV_LABELS = { k: f"🔹 {k}" for k in VIEW_CONFIG.keys() }

def render_current_view(tab_name, paciente_sel, mi_empresa, user, rol):
    if tab_name not in VIEW_CONFIG: return
    module_name, function_name = VIEW_CONFIG[tab_name]
    render_fn = getattr(import_module(module_name), function_name)
    
    if tab_name in ["Visitas y Agenda", "Recetas", "Caja", "PDF"]:
        render_fn(paciente_sel, mi_empresa, user, rol)
    elif tab_name in ["Admision", "Dashboard", "Inventario"]:
        render_fn(mi_empresa, rol)
    elif tab_name in ["Evolucion", "Estudios", "Red de Profesionales", "RRHH y Fichajes"]:
        render_fn(paciente_sel, user, rol)
    elif tab_name == "Mi Equipo":
        render_fn(mi_empresa, rol, user)
    elif tab_name in ["Clinica", "Telemedicina", "Historial"]:
        render_fn(paciente_sel)
    else:
        render_fn(paciente_sel, user)

# =====================================================================
# 6. LANDING PAGE (PUBLICIDAD ULTRA-PREMIUM)
# =====================================================================
if not st.session_state.get("entered_app", False):
    # Definimos el HTML en una variable para asegurar limpieza en el render
    publicidad_html = """
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;700;900&display=swap');
    .stApp { background-image: radial-gradient(circle at top right, #1e293b 0%, #020617 100%) !important; }
    #MainMenu, header, footer { visibility: hidden !important; }
    .landing-container { font-family: 'Inter', sans-serif; color: #f8fafc; display: flex; flex-direction: column; align-items: center; padding: 40px 20px; text-align: center; }
    .main-title { font-size: clamp(2.5rem, 6vw, 5rem); font-weight: 900; background: linear-gradient(135deg, #fff 30%, #38bdf8); -webkit-background-clip: text; -webkit-text-fill-color: transparent; line-height: 1.1; margin-bottom: 25px; }
    .stats-container { display: flex; gap: 30px; margin-bottom: 50px; flex-wrap: wrap; justify-content: center; }
    .stat-item { text-align: center; min-width: 100px; }
    .stat-num { font-size: 2.2rem; font-weight: 900; color: #38bdf8; display: block; }
    .stat-label { font-size: 0.8rem; color: #64748b; text-transform: uppercase; }
    .grid-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px; width: 100%; max-width: 1200px; margin-bottom: 60px; }
    .glass-card { background: rgba(255, 255, 255, 0.02); backdrop-filter: blur(10px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 30px; padding: 35px; transition: 0.4s; text-align: left; }
    .glass-card:hover { transform: translateY(-10px); border-color: #38bdf8; background: rgba(56, 189, 248, 0.05); }
    .contact-section { width: 100%; max-width: 1100px; padding: 60px 30px; background: #0f172a; border-radius: 40px; border: 1px solid rgba(56, 189, 248, 0.2); }
    .btn-wpp { display: block; background: #22c55e; color: white !important; padding: 12px; border-radius: 15px; text-decoration: none; font-weight: 700; margin-top: 15px; text-align: center; }
</style>
<div class="landing-container">
    <div style="background: linear-gradient(90deg, #38bdf8, #818cf8); color: #020617; padding: 5px 15px; border-radius: 50px; font-size: 0.8rem; font-weight: 800; margin-bottom: 20px; display:inline-block;">V9.12 ENTERPRISE PRO</div>
    <h1 class="main-title">Potencia tu Operación Médica<br>al Siguiente Nivel</h1>
    <div class="stats-container">
        <div class="stat-item"><span class="stat-num">100%</span><span class="stat-label">Digital</span></div>
        <div class="stat-item"><span class="stat-num">+50</span><span class="stat-label">Módulos</span></div>
        <div class="stat-item"><span class="stat-num">GPS</span><span class="stat-label">Real Time</span></div>
    </div>
    <div class="grid-cards">
        <div class="glass-card"><h3>🛰️ Geolocalización</h3><p>Auditoría satelital de visitas en tiempo real con respaldo GPS.</p></div>
        <div class="glass-card"><h3>📂 Historia Clínica</h3><p>Evoluciones multidisciplinarias y multimedia con blindaje legal.</p></div>
        <div class="glass-card"><h3>📦 Stock e Insumos</h3><p>Gestión automatizada con descuento por práctica realizada.</p></div>
        <div class="glass-card"><h3>🛡️ Auditoría Total</h3><p>Trazabilidad de cada acción para máxima seguridad institucional.</p></div>
    </div>
    <div class="contact-section">
        <h2 style="color:white; margin-bottom:30px;">Soporte Estratégico e Implementación</h2>
        <div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 20px;">
            <div style="background: rgba(0,0,0,0.2); padding: 25px; border-radius: 20px; flex: 1; min-width: 280px; border: 1px solid rgba(255,255,255,0.05);">
                <h4 style="color:#38bdf8; margin:0;">Enzo N. Girardi</h4>
                <a href="https://wa.me/5493584302024" target="_blank" class="btn-wpp">SOPORTE TÉCNICO 📲</a>
            </div>
            <div style="background: rgba(0,0,0,0.2); padding: 25px; border-radius: 20px; flex: 1; min-width: 280px; border: 1px solid rgba(255,255,255,0.05);">
                <h4 style="color:#38bdf8; margin:0;">Dario Lanfranco</h4>
                <a href="https://wa.me/5493584201263" target="_blank" class="btn-wpp">CONTRATACIONES 📲</a>
            </div>
        </div>
    </div>
</div>
"""
    # Renderizamos el HTML sin indentaciones que confundan al Markdown
    st.markdown(publicidad_html, unsafe_allow_html=True)
    
    st.markdown("<br>", unsafe_allow_html=True)
    if st.button("🚀 INGRESAR AL ECOSISTEMA", key="btn_ingresar_main", use_container_width=True):
        st.session_state.entered_app = True
        st.rerun()
    st.stop()

# --- 7. SEGURIDAD Y LOGIN ---
render_login()
check_inactividad()

user = st.session_state.get("u_actual")
if not user:
    st.stop()

mi_empresa, rol = user["empresa"], user["rol"]

# --- 8. BARRA LATERAL (GESTIÓN) ---
with st.sidebar:
    st.markdown(f"### {mi_empresa}\n**{user['nombre']}**")
    st.caption(f"Rol: {rol} - {descripcion_acceso_rol(rol)}")
    st.divider()
    
    # Obtener módulos permitidos (Lógica flexible del Core)
    menu = obtener_modulos_permitidos(rol)
    
    st.markdown("### 👤 Pacientes")
    buscar = st.text_input("Buscador rápido", placeholder="Nombre o DNI...")
    ver_altas = st.checkbox("Incluir Altas") if es_control_total(rol) else False
    
    p_f = obtener_pacientes_visibles(st.session_state, mi_empresa, rol, incluir_altas=ver_altas, busqueda=buscar)
    
    paciente_sel = None
    if p_f:
        paciente_actual = st.session_state.get("paciente_actual")
        opciones_ids = [i[0] for i in p_f]
        idx = opciones_ids.index(paciente_actual) if paciente_actual in opciones_ids else 0
        p_tuple = st.selectbox("Atención actual", p_f, index=idx, format_func=lambda x: x[1])
        paciente_sel = p_tuple[0]
        st.session_state["paciente_actual"] = paciente_sel
    
    if st.button("🚪 Cerrar Sesión", use_container_width=True):
        st.session_state.clear()
        st.rerun()

# --- 9. NAVEGACIÓN CENTRAL Y RENDERIZADO ---
if not menu:
    st.warning("⚠️ Su rol no tiene módulos asignados todavía. Contacte al administrador.")
else:
    vista_actual = st.session_state.get("modulo_actual", menu[0])
    if vista_actual not in menu: vista_actual = menu[0]
    
    selected = st.pills(
        "Módulos disponibles", 
        menu, 
        default=vista_actual, 
        format_func=lambda x: VIEW_NAV_LABELS.get(x, x),
        key="main_nav_pills"
    )
    
    if selected:
        st.session_state["modulo_actual"] = selected
        vista_actual = selected
    
    if paciente_sel:
        # Mini-header con info del paciente
        st.info(f"📍 **Atendiendo a:** {paciente_sel}")

    # Renderizar la vista correspondiente
    render_current_view(vista_actual, paciente_sel, mi_empresa, user, rol)
