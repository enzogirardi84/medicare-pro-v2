...
|---|---|---|---|
| **H1** | Streamlit Cloud está sirviendo el **`main.py` equivocado** (había entry points duplicados en el repo). El querystring `?modulo=Visitas+y+Agenda` no se resolvía correctamente. | `main.py` y `main_medicare.py` eran dos entry points distintos. Ya resuelto con `streamlit_app.py` como entry único. | **Alta** |
...
```
main.py              → entry point legacy (apuntaba al programa equivocado)
main_medicare.py     → entry point real del programa
main_login_fixed.py  → otro entry point legacy con st.set_page_config propio
```
- Si Streamlit Cloud estaba configurado con `main.py` como archivo principal, corría el entry point incorrecto.
- **Ya resuelto:** `streamlit_app.py` es el entry point canónico.
...
3. Unificar entry points: `streamlit_app.py` como entry point canónico. Eliminar `main.py` duplicado.
